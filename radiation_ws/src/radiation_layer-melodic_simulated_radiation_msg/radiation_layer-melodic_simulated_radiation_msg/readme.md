...
#### Added Functionality versus base

* Altered to work with the simulated radiation messages of the gazebo_radiation_plugin + a csiro specific version for reading data from the bag files. This version creates allows for the creation of a height map as well as the basic radiation map. 

* Service added (radiation_img_generator) for generating images of the radiation fields based on having a normal map (occupancy grid) and the radiation layer

* Service added (radiaiton_pointcloud_generator) for generating point clouds of the radiation based on the radiation layer and the height map produced.  

See Added Functionality Section of the documentation for details on how to use these services.
...
#### radiation_img_generator

For generating images of the radiation fields based on having a normal map (occupancy grid) and the radiation layer.

firstly run the service script using 
```
rosrun radiation_layer radiation_img_generator.py
```

This will make a rosservice called generate_radiation_image appear.

From here you can rosservice  call it passing in the arguments of:

* radiation_map_topic:  the topic name of the radiation layer being published 
  
* map_topic: the topic name of the map (occupancy grid) being published
  
* radiation_img_relative_save_locations: the location where the image file should be saved. **NB: Annoyingly this is relative to where the terminal running the service is not global. Should be addressed in the future.**



#### radiaiton_point_cloud_generator

For generating point clouds of the radiation based on the radiation layer and the height map produced.

firstly run the service script using 
```
rosrun radiation_layer radiation_point_cloud_generator.py
```

This will make a rosservice called generate_radiation_point_cloud appear.

From here you can rosservice  call it passing in the arguments of:

* radiation_map_topic: the topic name of the radiation layer being published 

* height_map_topic: the topic name of the height layer being published 

* radiation_pointcloud_topic: the topic on which to publish the point cloud 

* scale: the rosparam name of the z_scale for the height map 

* offset: the rosparam name of the z_offset for the height map

**NB: the z_scale and z_offset parameters can be changed form the reconfigure servers and allow for scaling of the height such that it can use all of the 0-254 range of costmap layer. This allows for more/less granular measurements in the z axis**
