#include<radiation_layer/height_layer.h>
...
PLUGINLIB_EXPORT_CLASS(height_layer_namespace::HeightLayer, costmap_2d::Layer)
...
namespace height_layer_namespace
{

HeightLayer::HeightLayer() {}

// How to Initialize the layer
void HeightLayer::onInitialize()
{
  ...
  nh.param("sensor_topic", sensor_topic, std::string("sensor_topic"));
  ...
  sensor_sub_ = g_nh.subscribe(sensor_topic, 1, &HeightLayer::sensorCB, this);
  layer_pub_ = nh.advertise<nav_msgs::OccupancyGrid>("height_map",100);

  // Add dynamic reconfigure to layer
  dsrv_ = new dynamic_reconfigure::Server<radiation_layer::HeightLayerConfig>(nh);
  dynamic_reconfigure::Server<radiation_layer::HeightLayerConfig>::CallbackType cb = boost::bind(
      &HeightLayer::reconfigureCB, this, _1, _2);
  dsrv_->setCallback(cb);
}


void HeightLayer::matchSize()
{
...
void HeightLayer::reconfigureCB(radiation_layer::HeightLayerConfig &config, uint32_t level)
{
  // If threshold values change, recompute entire costmap
  if (config.z_offset != z_offset_ || config.z_scale != z_scale_ || enabled_ != config.enabled ) {
    ...
    ROS_INFO("Updated: scale= %.2f offset= %.2f", z_scale_, z_offset_);
}

void HeightLayer::sensorCB(const gazebo_radiation_plugins::Simulated_Radiation_Msg& rad_msg)
{
  boost::recursive_mutex::scoped_lock lock(lock_);  // Ensure only this functon can interact with radiation sensor_msg_buffer_
  sensor_msg_buffer_.push_back(rad_msg);  // Place new message onto buffer
  boost::recursive_mutex::scoped_lock unlock(lock_);  // Release control of the buffer
}

void HeightLayer::updateObservations(std::list<std::pair<unsigned int, float> > &updates)
{
  std::list<gazebo_radiation_plugins::Simulated_Radiation_Msg> sensor_msg_buffer_copy_;  // Create blank buffer for observations
  ...
  sensor_msg_buffer_copy_ = std::list<gazebo_radiation_plugins::Simulated_Radiation_Msg>(sensor_msg_buffer_);  // Make copy of current observation buffer
  sensor_msg_buffer_.clear();  // Clear current observation buffer for new messages
  ...
  if (sensor_msg_buffer_copy_.size() == 0) {   //  If no observations are available for processing then return
    return;
  }

  // Loop over all observations
  for (std::list<gazebo_radiation_plugins::Simulated_Radiation_Msg>::iterator obs = sensor_msg_buffer_copy_.begin(); obs != sensor_msg_buffer_copy_.end(); obs++) {
    ...
    float value = obs -> pose.position.x;  // Retrive 2nd channel (index 1) from data field in message
...
void HeightLayer::getCache(std::list<std::pair<std::pair<double,double>, std::pair<float,float> > > &observation_cache)
{
  float* averages_copy;
...
unsigned int HeightLayer::scaledValue(float value)
{
  int scaled_value = value*z_scale_ +z_offset_;
  return (unsigned int)scaled_value;
}

void HeightLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x,
...
void HeightLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i,
                                          int max_j)
{
  if (!enabled_){
    return;
...
switch (combination_method_)
  ...
  publishLayer("height_layer");
  
}

void HeightLayer::updatePreserveNoInfo(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
...
std::vector<geometry_msgs::Point> HeightLayer::makeSensorFootprintFromParams(ros::NodeHandle& nh)
{
  ...
  if (nh.searchParam("sensor_radius", full_radius_param_name))
  {
    double sensor_radius;
    nh.param(full_radius_param_name, sensor_radius, 0.0);
    points = costmap_2d::makeFootprintFromRadius(sensor_radius);
    nh.setParam("sensor_radius", sensor_radius);
    inflate_radiation_ = true;
  }
  return points;
}

void HeightLayer::publishLayer(std::string layer_name){
    ...
    for (std::vector<boost::shared_ptr<costmap_2d::Layer> >::iterator pluginp = plugins->begin(); pluginp != plugins->end(); ++pluginp) {
      ...
      if(plugin->getName().find(layer_name)!=std::string::npos) {
          boost::shared_ptr<height_layer_namespace::HeightLayer> costmap;
          costmap = boost::static_pointer_cast<height_layer_namespace::HeightLayer>(plugin);
